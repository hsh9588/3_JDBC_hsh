...
public class BoardView {
			...
			Board board = bService.selectBoard(boardNo, MainView.loginMember.getMemberNo());  
	         ...
	         if (board != null) {
	             ...
	             System.out.printf("작성자 : %s | 작성일 : %s  \n조회수 : %d | 댓글수 : %d\n", 
	                   board.getMemberName(), board.getCreateDate(), board.getReadCount(), board.getCommentCount());
	             System.out.println("--------------------------------------------------------\n");
	 ...
	 * @param board(상세조회 된 게시글 + 작성자 번호 + 댓글 목록)
	 */
	private void subBoardMenu (Board board) {
		...
		try {
			...
			if (MainView.loginMember.getMemberNo() == board.getMemeberNo()) {
			...
			// 댓글 등록, 수정, 삭제 선택 시
			// 각가의 서비스 메서드 종료 후 다시 서브메뉴 메서드를 호출
			if (input > 0 && input < 5) {
	...
	private void insertComment(int bNo, int mNo) {
		...
		try {
			...
			// 내용 입력 받기
			String content = inputContent(); // 빈문자열
	...
	private void insertBoard() {
		...
		try {
			...
			board.setBoardTitle(boardTitle);
			board.setBoardContent(boardContent);
			board.setMemeberNo(MainView.loginMember.getMemberNo());
			...
			if (result > 0) {
				...
				if (b != null) {
					...
					System.out.printf("글번호 : %d \n제목 : %s\n", b.getBoardNo(), b.getBoardTitle());
					System.out.printf("작성자 : %s | 작성일 : %s  \n조회수 : %d | 댓글수 : %d\n", 
							b.getMemberName(), b.getCreateDate(), b.getReadCount(), b.getCommentCount());
					System.out.println("--------------------------------------------------------\n");
