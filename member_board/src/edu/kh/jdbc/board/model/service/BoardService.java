...
public class BoardService {
	...
	public List<Board> selectAllBoard() throws Exception {
		...
		Connection conn = JDBCTemplate.getConnection(); // JDBCTemplate -> import static 입력해놔서 생략 가능  
	...
	public Board selectBoard(int boardNo, int memberNo) throws Exception {
		...
		if (board != null) { // 게시글이 존재 한다면
			...
			if (memberNo != board.getMemeberNo()) {
	...
	public int deleteBoard(int boardNo) throws Exception {
		...
		int result = dao.updateBoard(conn, boardNo);
	...
	public int insertBoard(Board board) throws Exception {
		...
		board.setBoardNo(boardNo); // 얻어본 다음 번호를 board에 셋팅
		// -> 다음 게시글번호, 제목, 내용, 회원번호
